...
{   
	...
	public class TianDiTuLayer extends TiledMapServiceLayer
	{
		...
		override protected function getTileURL(level:Number, row:Number, col:Number) : URLRequest  
		{
			...
			{
				_baseURL = "http://59.255.48.160:81/img_c/wmts";
				_layerId = "img";
				_tileMatrixSetId = "c";
			}
			else if(this._mapStyle == "ImageCNNote")//获取影像地图（中文注记）
			{
				_baseURL = "http://59.255.48.160:81/cia_c/wmts";
				_layerId = "cia";
				_tileMatrixSetId = "c";
			}
			else if(this._mapStyle == "ImageENNote")//获取影像地图（英文注记）
			{
				_baseURL = "http://59.255.48.160:81/eia_c/wmts";
				_layerId = "eia";
				_tileMatrixSetId = "c";
			}
			else if(this._mapStyle == "TerrainBaseMap")//获取地形图（底图）
			{
				_baseURL = "http://59.255.48.160:81/ter_c/wmts";
				_layerId = "ter";
				_tileMatrixSetId = "c";
			}
			else if(this._mapStyle == "TerrainCNNote")//获取地形图（中文注记）
			{
				_baseURL = "http://59.255.48.160:81/cta_c/wmts";
				_layerId = "cta";
				_tileMatrixSetId = "c";
			}
			...
			{
				_baseURL = "http://59.255.48.160:81/vec_c/wmts";
				_layerId = "vec";
				_tileMatrixSetId = "c";
			}
			else if(this._mapStyle == "VectorCNNote")//获取矢量图（中文注记）
			{
				_baseURL = "http://59.255.48.160:81/cva_c/wmts";
				_layerId = "cva";
				_tileMatrixSetId = "c";
			}
			else if(this._mapStyle == "VectorENNote")//获取矢量图（英文注记）
			{
				_baseURL = "http://59.255.48.160:81/eva_c/wmts";
				_layerId = "eva";
				_tileMatrixSetId = "c";
			}
